...
/// Service for checking and managing Google Play In-App Updates on Android.
class UpdateService {
  /// Checks for available updates on the Google Play Store (Android only).
  ///
  /// - For immediate updates (critical/forced updates allowed by Play Store),
  ///   displays full screen update flow blocking app usage until updated.
  /// - For flexible updates (optional background updates), starts download and
  ///   completes update upon successful download.
    ...
    try {
      ...
      if (updateInfo.updateAvailability == UpdateAvailability.updateAvailable) {
        ...
        // 2. Otherwise, check if a flexible update is allowed
          ...
          debugPrint(
            "In-App Update: Starting flexible (background) update flow.",
          );
          final AppUpdateResult result =
