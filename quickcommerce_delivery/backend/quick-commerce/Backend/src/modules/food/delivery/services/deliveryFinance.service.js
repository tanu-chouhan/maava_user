...
// See riderEarningOf in delivery.service.js: riderEarning is never written, so
// pricing.deliveryFee is the figure the rest of the system already uses.
const RIDER_EARNING_EXPR_FINANCE = {
    $cond: [
...
export const getDeliveryPartnerWalletEnhanced = async (deliveryPartnerId) => {
    ...
    const [cashLimitSettings, earningsAgg, cashCollectedAgg, cashDepositsAgg, bonusAgg, withdrawalAgg, withdrawalsList, depositList, walletDoc] = await Promise.all([
        ...
        FoodOrder.aggregate([
            ...
            { $group: { _id: null, totalEarned: { $sum: RIDER_EARNING_EXPR_FINANCE } } }
        ]),
    ...
    const [ordersTx] = await Promise.all([
        FoodOrder.find({ 'dispatch.deliveryPartnerId': partnerId, orderStatus: 'delivered' })
            .sort({ createdAt: -1 })
            .select('orderId riderEarning pricing.deliveryFee payment orderStatus createdAt')
            .limit(20)
            .lean(),
    ]);
