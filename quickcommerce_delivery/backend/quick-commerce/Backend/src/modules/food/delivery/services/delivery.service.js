...
/**
 * What the partner actually earned on an order.
 *
 * `riderEarning` is declared on the order schema but NOTHING in the codebase
 * ever writes it — it stays at its default of 0 on every order. Summing it
 * therefore reported \u20b90 for a partner who had completed 13 deliveries, which
 * is what riders were seeing on the home screen.
 *
 * `pricing.deliveryFee` is the figure the rest of the system already treats as
 * the rider's cut: order.helpers.js falls back to exactly this when building
 * the new-order push, which is why the push showed \u20b950 while the earnings
 * screen showed \u20b90 for the same order.
 *
 * This is a READ-side repair, and it fixes orders already in the database. The
 * durable fix is to persist riderEarning when the order is created or
 * delivered; once that happens these fallbacks simply stop being reached.
 */
const RIDER_EARNING_EXPR = {
    $cond: [
...
/** The same rule for documents already loaded into JS. */
export const riderEarningOf = (order) =>
    Number(order?.riderEarning) || Number(order?.pricing?.deliveryFee) || 0;
...
export const getDeliveryPartnerWallet = async (deliveryPartnerId) => {
    ...
    const [earningsAgg, cashAgg] = await Promise.all([
        FoodOrder.aggregate([
            ...
            },
            {
                $group: {
                    _id: null,
                    totalEarned: { $sum: RIDER_EARNING_EXPR }
                }
            }
        ]),
        FoodOrder.aggregate([
            ...
            },
            {
                $group: {
                    _id: null,
                    cashInHand: { $sum: RIDER_EARNING_EXPR }
                }
            }
        ])
    ]);
    ...
    const [paymentTxList, bonusTxList] = await Promise.all([
        FoodOrder.find({
            ...
            .select('orderId riderEarning pricing.deliveryFee payment orderStatus deliveryState createdAt deliveryState.deliveredAt')
            .limit(2000)
            .lean(),
    ...
    const [totalOrders, agg] = await Promise.all([
        ...
        FoodOrder.aggregate([
            { $match: match },
            {
                $group: {
                    _id: null,
                    totalEarnings: { $sum: RIDER_EARNING_EXPR }
                }
            }
        ])
    ]);
...
const toTripDto = (order) => {
    ...
    const earningAmount = riderEarningOf(order) || Number(order?.deliveryEarning) || 0;
