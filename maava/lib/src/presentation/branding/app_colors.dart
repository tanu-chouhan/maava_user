...
class AppColors {
  ...
  static Color primaryButton = const Color(0xFFA78BFA);

  // ==================== DERIVED BRAND SHADES ====================
  // Getters, not fields: each is recomputed from the *current* [primary] on
  // every read, so the tints/shades that used to be hardcoded brand hexes
  ...
  // does not produce garish pale tints, and a muted one still reads as tinted
  // rather than grey.
  ...
  ///
  /// Relative to the primary's own lightness, not a fixed value: the brand
  /// violet sits at L≈0.66, so the old absolute 0.62 produced a shade *darker*
  /// than the primary and inverted the meaning of "light". The +0.12 offset
  /// reproduces the previous value exactly for a mid-lightness brand colour
  /// (the old orange sat at L=0.50 → 0.62), so no other preset shifts.
  static Color get primaryLight =>
      _shade((_hsl.lightness + 0.12).clamp(0.0, 0.80), _hsl.saturation);
  ...
  /// to be written as a hardcoded translucent brand hex or
  /// `primary.withOpacity(...)`.
