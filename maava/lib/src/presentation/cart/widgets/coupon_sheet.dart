...
class CouponSheet {
  ...
  /// Resolves to the accepted coupon so the cart can celebrate once this sheet
  /// has closed; null when nothing was applied.
  static Future<CouponWin?> show(BuildContext context) {
    return showModalBottomSheet<CouponWin>(
      context: context,
...
class _CouponSheetBodyState extends ConsumerState<_CouponSheetBody> {
  ...
  Future<void> _apply(String code) async {
    ...
    // ignore: avoid_print
    AppLogger.debug(scope: 'cart', '[COUPON] Applying code: ${code.trim().toUpperCase()}');
    ...
    if (applied) {
      // The celebration replaces the old success snackbar — the cart shows it
      // once this sheet is gone, using the figures the server echoed back.
      Navigator.of(context).pop(
        CouponWin(
          code: pricing!.couponCode ?? code.trim().toUpperCase(),
