...
class CheckoutViewModel extends Notifier<CheckoutState> {
  ...
  Future<void> applyCoupon(String code) async {
    ...
    await recalculate();
    // ignore: avoid_print
    AppLogger.debug(scope: 'checkout', 
  ...
  Future<void> removeCoupon() async {
    ...
    await recalculate();
    // ignore: avoid_print
    AppLogger.debug(scope: 'checkout', '[COUPON] removed, total=${state.pricing?.total}');
  }
  ...
  Future<PaymentFlowResult> payAndPlaceOrder({
    ...
    // For non-gateway payment methods (Cash on Delivery & MAAVA Wallet)
    if (paymentMethod == 'cash' || paymentMethod == 'wallet') {
      ...
      return PaymentFlowResult(
        ...
        message: paymentMethod == 'cash'
            ...
            : 'Order paid successfully using MAAVA Wallet 🎉',
        orderId: orderId,
      );
    }
