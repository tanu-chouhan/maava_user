...
abstract final class CartActions {
  ...
  static Future<void> _commit(
    ...
    switch (outcome) {
      ...
      case SellerConflict():
        // One order is fulfilled by one outlet (Order.restaurantId is required
        // on the backend), so a basket cannot span two. That is a delivery
        // constraint, not a marketplace choice between merchants — the copy
        // says so, and never names it as picking a different seller.
        final replace = await AppDialog.confirm(
          context,
          icon: Icons.storefront_outlined,
          title: 'Start a new cart?',
          message:
              'This item is stocked at a different store. Adding it will clear '
              'your current cart so everything still arrives in one delivery.',
