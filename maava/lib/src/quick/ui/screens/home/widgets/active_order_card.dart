...
class _ActiveOrderFloatingCardState
  ...
  void initState() {
    ...
    _refreshTimer = Timer.periodic(const Duration(seconds: 8), (_) {
      if (!mounted) return;
      // Only poll while there is actually something to track. Without this the
      // card hit GET /quick/orders every 8s for the whole time Quick home was
      // on screen — signed out, or with no active order and the card rendering
      // nothing. It was by far the busiest endpoint on the server. The food
      // side's ActiveOrderViewModel already gates its timer this way.
      if (ref.read(ordersProvider).active.isEmpty) return;
