...
class CatalogGroupingService {
  ...
  /// One tile per distinct product, keeping the cheapest listing.
  ///
  /// The catalogue carries one row per seller, so a product stocked by three
  /// stores arrives as three products with the same name at different prices.
  /// Ranked home rows are about *what* to buy, not who from, so showing the
  /// same item three times just crowds out the rest of the catalogue — and
  /// reads as a bug. Cheapest wins, which is also what the shopper wants.
  ///
  /// Seller-scoped surfaces deliberately do NOT use this: on a store's own
  /// page every listing is that store's own.
  List<Product> distinctByName(List<Product> products) {
    final best = <String, Product>{};
    for (final p in products) {
      final key = '${_slug(p.name)}|${_slug(p.brand)}|${_slug(p.packSize)}';
      final existing = best[key];
      if (existing == null || p.price < existing.price) best[key] = p;
    }
  ...
  List<Product> bestSellers(List<Product> products, {int take = 10}) {
    final list = distinctByName(products)..sort((a, b) {
  ...
  List<Product> flashSale(List<Product> products, {int take = 10}) {
    final list = distinctByName(products)
